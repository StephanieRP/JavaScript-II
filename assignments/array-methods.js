...
runners.forEach(function(name) {
    let names = name.first_name + " " + name.last_name;
    fullName.push(names)
    return fullName;
})
...
let allCaps = [];
runners.map(function (firstCap) {
    let cap = firstCap.first_name.toUpperCase();
    allCaps.push(cap);
})
console.log(allCaps); 
...
let ticketPriceTotal = runners.reduce(function(total,amount) {
    return total += amount.donation;
    //  ticketPriceTotal.push(total)
    // total = ticketPriceTotal;
}, 0)
...
// Problem 1
// The big boss wants to find all runners who donated more than $250 and give them a "free" gift. 
let generious = []
let bigSpender = runners.filter(function(donatedAmount){
    if(donatedAmount.donation >= 250) {
        return donatedAmount.donation
    }
}).map( function (displayMessage) {
    let message = `${displayMessage.first_name} ${displayMessage.last_name} is awesome`;
         generious.push(message)
         return message
})
...
// Problem 2
// Need to find all runners who work for the Skinix company
let sameCompany = []
let same = runners.filter(function (companyName) {
    if(companyName.company_name === 'Skinix') {
        sameCompany.push(companyName)
        return sameCompany
    }
})
...
// Problem 3
//Big boss just wants runners whose last start with letters A-D, and display their information
